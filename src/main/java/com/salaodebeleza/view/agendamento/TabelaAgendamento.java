...
public class TabelaAgendamento extends JInternalFrame {
	...
	private TableRowSorter<TabelaAgendamentoModel> sortTabelaAgendamento;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					TabelaAgendamento frame = new TabelaAgendamento();
	...
	private void initComponents() {
		...
		btnIncluir.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/book_add.png")));
		
		btnAlterar = new JButton("Alterar");
		
		btnAlterar.setMnemonic(KeyEvent.VK_A);
		btnAlterar.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/book_edit.png")));
		
		btnExcluir = new JButton("Excluir");
		
		btnExcluir.setMnemonic(KeyEvent.VK_E);
		btnExcluir.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/book_delete.png")));
		
		btnSair = new JButton("Sair");
		btnSair.setMnemonic(KeyEvent.VK_S);
		
		btnSair.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/sair.png")));
		...
		btnPesquisar.setToolTipText("Pesquisar agendamento cadastrado\r\n");
		btnPesquisar.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/search.png")));
		...
		btnPrimeiro.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/go-first.png")));
		
		btnAnterior = new JButton("");
		
		btnAnterior.setToolTipText("Anterior");
		btnAnterior.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/go-previous.png")));
		
		btnProximo = new JButton("");
		
		btnProximo.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/go-next.png")));
		btnProximo.setToolTipText("Pr\u00F3ximo");
		
		btnUltimo = new JButton("");
		
		btnUltimo.setIcon(new ImageIcon(TabelaAgendamento.class.getResource("/com/salaodebeleza/estrutura/imagens/go-last.png")));
		...
		scrollPane.setViewportView(tabelaAgendamento);
	...
	private void fitraNomeUsuario(String filtro) {
		RowFilter<TabelaAgendamentoModel, Object> rowFilter = null;
		try {
		...
		sortTabelaAgendamento.setRowFilter(rowFilter);
	}
	
	private void incluirAgendamento() {
		AgendamentoGUI usuario = new AgendamentoGUI(new JFrame(), true, tabelaAgendamento, tabelaAgendamentoModel, 0, VariaveisProjeto.INCLUSAO);
	...
	private void alterarAgendamento() {
		if(tabelaAgendamento.getSelectedRow()!= -1 && tabelaAgendamento.getSelectedRow() < tabelaAgendamentoModel.getRowCount()) {
			int linha = tabelaAgendamento.getSelectedRow();
			AgendamentoGUI usuario = new AgendamentoGUI(new JFrame(), true, tabelaAgendamento, tabelaAgendamentoModel, linha, VariaveisProjeto.ALTERACAO);
			usuario.setLocationRelativeTo(null);
			usuario.setVisible(true);
		}
	...
	private void excluirAgendamento() {
		if(tabelaAgendamento.getSelectedRow()!= -1 && tabelaAgendamento.getSelectedRow() < tabelaAgendamentoModel.getRowCount()) {
			int linha = tabelaAgendamento.getSelectedRow();
			AgendamentoGUI usuario = new AgendamentoGUI(new JFrame(), true, tabelaAgendamento, tabelaAgendamentoModel, linha, VariaveisProjeto.EXCLUSAO);
			usuario.setLocationRelativeTo(null);
			usuario.setVisible(true);
		}
	...
	protected void iniciaPaginacao() {
			...
			tabelaAgendamentoModel = new TabelaAgendamentoModel();
			
			tabelaAgendamentoModel.setListaAgendamento(carregaListaAgendamento(numeroPagina, defaultPagina));
			
			tabelaAgendamento.setModel(tabelaAgendamentoModel);
			
			tabelaAgendamento.setFillsViewportHeight(true);
			
			tabelaAgendamento.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
			
			tabelaAgendamentoModel.fireTableDataChanged();
			
			sortTabelaAgendamento = new TableRowSorter<TabelaAgendamentoModel>(tabelaAgendamentoModel);
			
			tabelaAgendamento.setRowSorter(sortTabelaAgendamento);
			
				
			tabelaAgendamento.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
			
			
			tabelaAgendamento.getColumnModel().getColumn(CODIGO).setWidth(11);
			tabelaAgendamento.getColumnModel().getColumn(NOME).setWidth(100);
	...
	private List<Agendamento> carregaListaAgendamento(Integer numeroPagina, Integer defaultPagina) {

		AgendamentoService agendamentoService = new AgendamentoService();

		List<Agendamento> listaAgendamento  = new ArrayList<Agendamento>();
		
		listaAgendamento = agendamentoService.listAgendamentosPaginacao( ( defaultPagina * (numeroPagina - 1 )), defaultPagina);
	...
	private Integer buscaTotalRegistroUsuario() {
		...
		totalRegistro = agendamentoService.countTotalRegister();
